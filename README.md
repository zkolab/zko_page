...
- `voice.html` / `voice-free.html` / `voice-premium.html` / `voice.css` / `voice.js`：AutoClipboard 语音版本选择首页、免费版与高级版介绍，以及写作模式交互演示
...
- 全站右上角账户区展示头像和用户名；用户名同时用于登录、网页展示和 AutoClipboard 账户显示。账户中心把“注册账号”和“登录已有账号”明确分开，注册时一次填写邮箱、用户名和密码，邮箱验证码通过后由 CloudBase Auth 创建账户；已有用户可使用邮箱或用户名直接配合密码登录，已绑定手机号的账户也可使用手机号和密码，邮箱验证码保留为备用免密码入口。个人资料会在再次登录后自动回填；头像使用独立的可移动、可缩放裁剪框，确认后立即保存并同步替换右上角头像，且不要求重复提交用户名。修改密码使用 CloudBase Auth 邮箱验证码，并在账户页输入验证码与两次新密码后完成；业务数据库不保存密码。
